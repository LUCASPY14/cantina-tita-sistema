...
class TestModuloConciliacion:
    ...
    def test_01_registrar_conciliacion(self):
        ...
        try:
            ...
            self.cursor.execute("""
                SELECT pv.ID_Pago, pv.Monto_Aplicado, mp.Nombre
                FROM pagos_venta pv
                INNER JOIN medios_pago mp ON pv.ID_Medio_Pago = mp.ID_Medio_Pago
                WHERE mp.Nombre IN ('Tarjeta de Credito', 'Tarjeta de Debito', 'Giros TIGO')
                  AND pv.Monto_Aplicado > 0
                  AND NOT EXISTS (
                      SELECT 1 FROM conciliacion_pagos cp 
                      WHERE cp.ID_Pago_Venta = pv.ID_Pago
                  )
                LIMIT 1
            """)
    ...
    def test_02_identificar_diferencias(self):
        ...
        try:
            ...
            self.cursor.execute("""
                SELECT pv.ID_Pago, pv.Monto_Aplicado
                FROM pagos_venta pv
                INNER JOIN medios_pago mp ON pv.ID_Medio_Pago = mp.ID_Medio_Pago
                WHERE mp.Nombre IN ('Tarjeta de Credito', 'Tarjeta de Debito')
                  AND pv.Monto_Aplicado > 0
                  AND NOT EXISTS (
                      SELECT 1 FROM conciliacion_pagos cp 
                      WHERE cp.ID_Pago_Venta = pv.ID_Pago
    ...
    def test_03_consultar_por_periodo(self):
        ...
        try:
            ...
            self.cursor.execute("""
                ...
                INNER JOIN pagos_venta pv ON cp.ID_Pago_Venta = pv.ID_Pago
                INNER JOIN medios_pago mp ON pv.ID_Medio_Pago = mp.ID_Medio_Pago
