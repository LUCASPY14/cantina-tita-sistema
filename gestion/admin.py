...
class DatosEmpresaAdmin(admin.ModelAdmin):
    list_display = ['razon_social', 'nombre_fantasia', 'ruc', 'telefono', 'activo']
...
class CostosHistoricosAdmin(admin.ModelAdmin):
    list_display = ['id_producto', 'costo_unitario', 'fecha_costo', 'id_compra']
    list_filter = ['fecha_costo']
    search_fields = ['id_producto__descripcion', 'id_producto__codigo']
...
class ComprasAdmin(admin.ModelAdmin):
    list_display = ['id_compra', 'id_proveedor', 'fecha_compra', 'monto_total_con_iva', 'estado']
    list_filter = ['estado', 'fecha_compra']
    search_fields = ['nro_factura_proveedor', 'id_proveedor__razon_social']
...
class CtaCorrienteProvAdmin(admin.ModelAdmin):
    list_display = ['id_proveedor', 'tipo_movimiento', 'monto', 'fecha_movimiento', 'saldo_resultante']
    list_filter = ['tipo_movimiento', 'fecha_movimiento']
    search_fields = ['id_proveedor__razon_social']


@admin.register(CargasSaldo)
class CargasSaldoAdmin(admin.ModelAdmin):
    list_display = ['nro_tarjeta', 'monto_cargado', 'fecha_carga', 'saldo_anterior', 'saldo_nuevo']
    list_filter = ['fecha_carga']
    search_fields = ['nro_tarjeta__nro_tarjeta', 'id_cliente_autorizo__nombres']
...
class DatosFacturacionElectAdmin(admin.ModelAdmin):
    list_display = ['id_documento', 'cdc', 'estado_sifen', 'fecha_envio_sifen']
...
class DatosFacturacionFisicaAdmin(admin.ModelAdmin):
    list_display = ['id_documento', 'nro_factura_completo', 'id_empleado_emitio']
    search_fields = ['nro_factura_completo']
...
class AjustesInventarioAdmin(admin.ModelAdmin):
    list_display = ['id_ajuste', 'tipo_ajuste', 'fecha_ajuste', 'id_empleado_autoriza']
    list_filter = ['tipo_ajuste', 'fecha_ajuste']
...
class CajasAdmin(admin.ModelAdmin):
    list_display = ['nombre_caja', 'id_punto', 'estado', 'activo']
    list_filter = ['estado', 'activo']


@admin.register(CierresCaja)
class CierresCajaAdmin(admin.ModelAdmin):
    list_display = ['id_caja', 'fecha_apertura', 'fecha_cierre', 'monto_inicial', 'monto_final_sistema', 'diferencia']
...
class ConciliacionPagosAdmin(admin.ModelAdmin):
    list_display = ['id_pago_venta', 'fecha_conciliacion', 'monto_conciliado', 'estado']
    list_filter = ['estado', 'fecha_conciliacion']


@admin.register(CtaCorriente)
class CtaCorrienteAdmin(admin.ModelAdmin):
    list_display = ['id_cliente', 'tipo_movimiento', 'monto', 'fecha_movimiento', 'saldo_resultante']
    list_filter = ['tipo_movimiento', 'fecha_movimiento']
    search_fields = ['id_cliente__nombres', 'id_cliente__apellidos']
...
class AlertasSistemaAdmin(admin.ModelAdmin):
    list_display = ['tipo_alerta', 'descripcion', 'fecha_generacion', 'estado', 'fecha_resolucion']
    list_filter = ['tipo_alerta', 'estado', 'fecha_generacion']
...
class AuditoriaEmpleadosAdmin(admin.ModelAdmin):
    list_display = ['id_empleado', 'accion', 'tabla_afectada', 'fecha_hora']
    list_filter = ['tabla_afectada', 'fecha_hora']
    search_fields = ['id_empleado__nombre', 'accion']


@admin.register(AuditoriaUsuariosWeb)
class AuditoriaUsuariosWebAdmin(admin.ModelAdmin):
    list_display = ['id_cliente', 'accion', 'fecha_hora', 'ip_address']
    list_filter = ['fecha_hora']
    search_fields = ['id_cliente__nombres', 'accion', 'ip_address']


@admin.register(AuditoriaComisiones)
class AuditoriaComisionesAdmin(admin.ModelAdmin):
    list_display = ['id_detalle_comision', 'monto_venta', 'porcentaje_aplicado', 'monto_comision', 'fecha_calculo']
